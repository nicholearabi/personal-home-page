...
document.getElementById('enter-btn').addEventListener('click', (event) => {
  event.preventDefault();
  comments.push(document.getElementById('comment').value);
  let element_list_comment = document.getElementById('list-comment');
  let new_li = document.createElement('li');
  new_li.innerText = comments.slice(-1)[0];

  element_list_comment.appendChild(new_li);
